...
@inline function _edge_slope_estimates(
    ...
    m0 =
        (y0m !== nothing && isfinite(state.U[y0m])) ? (state.U[y1] - state.U[y0m]) / 2 :
        T(NaN)
    m1 =
        (y1p !== nothing && isfinite(state.U[y1p])) ? (state.U[y1p] - state.U[y0]) / 2 :
        T(NaN)
    return (m0, m1)
end
...
@inline function _triangle_minimum(
    ...
    for _ = 1:8
        ...
        Hxy =
            (
                P(λ1 + h, λ2 + h) - P(λ1 + h, λ2 - h) - P(λ1 - h, λ2 + h) +
                P(λ1 - h, λ2 - h)
            ) / (4h^2)
        det = Hxx * Hyy - Hxy^2
        abs(det) < eps(T) && break
...
function _local_update(
    ...
    best, best_ref =
        _add_simplex_candidates(best, best_ref, x, c_x, state, grid, L, Val(K), Val(D))
...
function _add_simplex_candidates(
    ...
    @inbounds for δ0 in offsets
        ...
        for δch in cheb
            ...
            Φ, λstar = _edge_minimum(c_x, c_y0, c_y1, U0, U1, m0, m1, L, nd_x, best, Φ0, Φ1)
            if Φ < best
                best = Φ
