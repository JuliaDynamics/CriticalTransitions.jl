...
const _OLIM_ROTATED_NULL_MSG =
    ...
    "null space, which is not supported. " *
    _RANK_DEFICIENT_MSG
...
function _degenerate_split(a0::AbstractMatrix{T}; atol::T = sqrt(eps(real(T)))) where {T}
    ...
    is_zero_rowcol(i) =
        all(j -> abs(a0[i, j]) <= atol, 1:D) && all(j -> abs(a0[j, i]) <= atol, 1:D)
...
_make_Q_inv(a_fn, ::Val{D}, ::Type{T}) where {D,T} = _QInvDynamic{D,T,typeof(a_fn)}(a_fn)
...
function _geometric_lagrangian(
    sys::CoupledSDEs{IIP,D},
    ::Type{T};
    eps_b::T = T(1.0e-10),
    regularization::Real = zero(T),
) where {IIP,D,T}
    raw = dynamic_rule(sys);
    p = current_parameters(sys)
    b = if IIP
    ...
    Q_inv =
        a_fn isa Base.Returns ? inv(a0 + δ * _onehot_diag(Val(D), z, T)) :
