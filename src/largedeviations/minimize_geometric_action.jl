...
function _gmam_setup(sys::CoupledSDEs, init)
    ...
    S =
        let b_fn = b_fn,
            ...
            A_at = A_at,
            v_buf = v_buf,
            integrand_buf = integrand_buf

            x -> _geometric_action_from_drift!(
                b_fn,
                fix_ends(x, x_i, x_f),
                1.0,
                A_at,
...
function _lambda_theta!(θ_out, a_i, b_i, φp, Ainv_b, Ainv_φp, F_cached = nothing)
    if a_i isa LinearAlgebra.Diagonal
        ...
        F =
            F_cached === nothing ? LinearAlgebra.lu(a_i isa Matrix ? a_i : Matrix(a_i)) :
            F_cached
        copyto!(Ainv_b, b_i);
        LinearAlgebra.ldiv!(F, Ainv_b)
        copyto!(Ainv_φp, φp);
        LinearAlgebra.ldiv!(F, Ainv_φp)
    end
    num = dot(b_i, Ainv_b)
    den = dot(φp, Ainv_φp)
