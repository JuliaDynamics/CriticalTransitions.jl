...
function _find_nzval_idx(M::SparseArrays.SparseMatrixCSC, r::Int, c::Int)
    @inbounds for k = M.colptr[c]:(M.colptr[c+1]-1)
        if M.rowval[k] == r
            return k
        end
    end
...
function _build_coupled_cache(sys, ::Type{T}, Nx::Int, Nt::Int) where {T}
    ...
    a_at = [Matrix{T}(undef, Nx, Nx) for _ = 1:Nt]
    p_zero = zeros(T, Nx, Nt)
    Hp_buf = Matrix{T}(undef, Nx, Nt)
    Hx_buf = Matrix{T}(undef, Nx, Nt)
...
function update_p!(p, λ, x, xdot, sys, cache::SgMAMCoupledCache)
    ...
    @inbounds for t in axes(x, 2)
        ...
        bt = view(b_, :, t);
        xt = view(xdot, :, t)
        copyto!(cache.Ainv_b, bt);
        LinearAlgebra.ldiv!(F, cache.Ainv_b)
        copyto!(cache.Ainv_xd, xt);
        LinearAlgebra.ldiv!(F, cache.Ainv_xd)
...
function update_x!(
    ...
    for dof = 1:Nx
        ...
        @inbounds for k = 1:L
            ...
            ia = a_inv[dof, i]
            rhs[k] =
                x[dof, i] + ϵ * (λ[i] * p′[dof, i] + Hx[dof, i] - ia * λ[i]^2 * x′′[dof, i])
        end
