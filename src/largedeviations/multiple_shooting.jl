...
function MultipleShooting(;
    ...
    nshoots ≥ 2 ||
        throw(ArgumentError("MultipleShooting requires nshoots ≥ 2; got $nshoots"))
    T = promote_type(
        typeof(float(abstol)),
        typeof(float(reltol)),
        typeof(float(eps_lin)),
        typeof(float(invariant_tol)),
    )
...
function _hamiltonian_value(
    H::FreidlinWentzellHamiltonian,
    x::AbstractVector,
    p::AbstractVector,
)
    b = _drift(H, x)
...
function _integrate_segment(
    ws::MultipleShootingWorkspace{IIP,D},
    y_in,
    s_a,
    s_b,
    L,
...
function minimize_geometric_action(
    ...
    if H_inv_max > ws.invariant_tol
        @warn "MultipleShooting: H=0 invariant violated" H_invariant_max = H_inv_max threshold =
            ws.invariant_tol
    end
    b_fn = x -> _drift(sys, x)
    ...
    action = _geometric_action_from_drift!(
        b_fn,
        path_mat,
        one(eltype(path_mat)),
        A_at,
    ...
    show_progress &&
        @info "MultipleShooting converged" residual = resnorm iterations = niter path_length =
            L action = action
    return MinimumActionPath(
        StateSpaceSet(Matrix(path_mat')),
        action;
        λ = L,
        generalized_momentum = Matrix(p_mat'),
    )
end
