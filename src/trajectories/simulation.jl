...
This function is equivalent to calling [`trajectory`](@extref DynamicalSystemsBase.trajectory) on the deterministic part of the
...
* `kwargs...`: keyword arguments passed to [`trajectory`](@extref DynamicalSystemsBase.trajectory)

For more info, see [`ODEProblem`](https://diffeq.sciml.ai/stable/types/ode_types/#SciMLBase.ODEProblem).
For stochastic integration, see [`trajectory`](@extref DynamicalSystemsBase.trajectory).

"""
function deterministic_orbit(
    sys::CoupledSDEs,
    T,
    init = current_state(sys);
    diffeq = CoupledODEs(sys).diffeq,
    kwargs...,
)
