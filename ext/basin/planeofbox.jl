...
function vthreshold(u, A, B, C, box)
    ...
    for ii in 1:length(A)
        if V[ii] ≠ 0
            vals = (
                (box[ii].lo - A[ii] - u[1] * U[ii]) / V[ii],
                (box[ii].hi - A[ii] - u[1] * U[ii]) / V[ii],
            )
            low = minimum(vals)
            high = maximum(vals)
            ϕ₁ = push!(ϕ₁, low .. high)
    ...
    for ii in 1:length(A)
        if V[ii] ≠ 0
            vals = (
                (box[ii].lo - A[ii] - u[2] * U[ii]) / V[ii],
                (box[ii].hi - A[ii] - u[2] * U[ii]) / V[ii],
            )
            low = minimum(vals)
            high = maximum(vals)
            ϕ₂ = push!(ϕ₂, low .. high)
...
function plane(A, B, C, box; step::Vector=[0.1, 0.1])
    ...
    elseif A == B || A == C
        ...
        ϕₜ = [] # here i should define an empty vector in a better way! i'm later going to fill it with Interval{Real}s

        Threads.@threads for ii in 1:d
            if A[ii] ≠ B[ii]
                vals = (
                    (box[ii].lo - A[ii]) / (B[ii] - A[ii]),
                    (box[ii].hi - A[ii]) / (B[ii] - A[ii]),
                )
                low = minimum(vals)
                high = maximum(vals)
        ...
        V = [[] for i in 1:length(U₁), j in 1:length(U₂)]

        Threads.@threads for ii in 1:length(U₁)
            us = [[U₁[ii], u] for u in U₂] # the candidate for umin and umax
            vs = vthreshold.(us, Ref(A), Ref(B), Ref(C), Ref(box))
            V[ii, :] = [
                [
                    [(U₁[ii], U₂[jj]), (vs[jj][1], vs[jj][2])],
                    (vs[jj][2] - vs[jj][1]) * (U₂[jj] - U₁[ii]),
                ] for jj in 1:length(U₂)
            ]
        end

        W = [V[ii, jj][2] for ii in 1:size(V, 1), jj in 1:size(V, 2)]
...
#         ϕₛ = [] # here i should define an empty vector in a better way! i'm later going to fill it with Interval{Real}s
#         if A ≠ C
#             @Threads.threads for ii ∈ 1:d
#                 if A[ii] ≠ C[ii]
