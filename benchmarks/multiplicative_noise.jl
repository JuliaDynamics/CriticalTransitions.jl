function benchmark_multiplicative_noise!(SUITE)
    let
        ...
        SUITE["Large deviation"]["Simple geometric minimal action"]["multiplicative diagonal"] =
        ...
        SUITE["Large deviation"]["Geometric minimal action"]["multiplicative diagonal"] =
            @benchmarkable minimize_geometric_action(
                $ds,
                $x_initial,
                GeometricGradient();
    ...
    let
        ...
        sys = FreidlinWentzellHamiltonian(ds)
        opt = GeometricGradient(; stepsize = 1.0)
        SUITE["Large deviation"]["Simple geometric minimal action"]["multiplicative off-diagonal"] =
            @benchmarkable minimize_geometric_action(
                $sys,
                $x_initial,
                $opt;
        ...
        SUITE["Large deviation"]["Geometric minimal action"]["multiplicative off-diagonal"] =
            @benchmarkable minimize_geometric_action(
                $ds,
                $x_initial,
                GeometricGradient();
