...
bistable_H() =
    FreidlinWentzellHamiltonian(CoupledSDEs(bistable_rule, [0.0]; noise_strength = 1.0))
maier_stein_H() = FreidlinWentzellHamiltonian(
    CoupledSDEs(maier_stein_rule, [0.0, 0.0]; noise_strength = 1.0),
)

linear_ramp(x_start, x_end, Nt) =
    Matrix(reshape(collect(range(x_start, x_end; length = Nt)), 1, Nt))

maier_stein_arc(x_start, x_end, Nt; amp = 0.2) =
    let
...
run_silent(args...; kwargs...) =
    with_logger(NullLogger()) do
        minimize_geometric_action(args...; kwargs..., show_progress = false)
    end

H_invariant_max(H, res) =
    let D = size(res.generalized_momentum, 2), N = length(res.path)
        maximum(
            abs(
                CT._hamiltonian_value(
                    H,
                    [res.path[i][k] for k = 1:D],
                    [res.generalized_momentum[i, k] for k = 1:D],
                ),
            ) for i = 1:N
        )
    end
...
@testset "1D bistable additive: attractor → saddle (half instanton)" begin
    ...
    cfg = [res.path[k][1] for k = 1:length(res.path)]
    ...
    ref_action =
        CT._geometric_action_from_drift(b_fn, linear_ramp(-1.0, 0.0, N_ref), 1.0, A_at)
...
@testset "1D bistable: full instanton via splitting (no interior fp)" begin
    ...
    res_l = run_silent(
        H,
        linear_ramp(-1.0, 0.0, Nt),
        MultipleShooting(; nshoots = 6, maxiters = 200),
    )
...
@testset "Type stability" begin
    ...
    @test (@inferred CT._build_workspace(H, x_init, opt)) isa
          CT.MultipleShootingWorkspace{<:Any,1}
end
