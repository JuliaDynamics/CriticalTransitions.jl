...
bistable_system() = CoupledSDEs(bistable_rule, [0.0]; noise_strength = 1.0)
maier_stein_system() = CoupledSDEs(maier_stein_rule, [0.0, 0.0]; noise_strength = 1.0)
...
@testset "1D bistable additive: attractor → saddle" begin
    ds = bistable_system()
    ...
    ref_action = geometric_action(ds, linear_ramp(-1.0, 0.0, 10_000), 1.0)
    @test isapprox(res.action, ref_action; rtol = 5.0e-3)
    @test isapprox(res.λ, 1.0; rtol = 1.0e-2)
end

@testset "1D bistable: full instanton via splitting" begin
    ds = bistable_system()
    H = FreidlinWentzellHamiltonian(ds)
    Nt = 20
    ...
    ref_action_half = geometric_action(ds, linear_ramp(-1.0, 0.0, 10_000), 1.0)
    @test isapprox(res_l.action + res_r.action, 2 * ref_action_half; rtol = 5.0e-3)
end

@testset "Convergence in nshoots" begin
    H = FreidlinWentzellHamiltonian(bistable_system())
...
@testset "Maier-Stein additive: shooting matches sgMAM" begin
    H_ms = FreidlinWentzellHamiltonian(maier_stein_system())
...
@testset "1D bistable multiplicative" begin
    γ = 0.5
    ...
    ref_action = geometric_action(ds, linear_ramp(-1.0, 0.0, 5_000), 1.0)
    @test isapprox(res.action, ref_action; rtol = 5.0e-3)
end
...
@testset "Generalized momentum shape" begin
    H = FreidlinWentzellHamiltonian(bistable_system())
    Nt = 30
