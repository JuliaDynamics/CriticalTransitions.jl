...
@testset "_validate_and_classify_a: CoupledSDEs" begin
    ...
    f1d(u, p, t) = SA[-u[1]]
    ds_diagmult = CoupledSDEs(f1d, SA[1.0]; g = g1d, noise_prototype = SMatrix{1,1}(0.0))
    ...
    ds_gen =
        CoupledSDEs(f2, SA[1.0, 0.0]; g = g2, noise_prototype = SMatrix{2,2}(zeros(2, 2)))
...
@testset "proper_FW_system" begin
    ...
    g1d(u, p, t) = SA[sqrt(1 + 0.3 * u[1]^2);;]
    f1d(u, p, t) = SA[-u[1]]
    ds_mult = CoupledSDEs(f1d, SA[1.0]; g = g1d, noise_prototype = SMatrix{1,1}(0.0))
