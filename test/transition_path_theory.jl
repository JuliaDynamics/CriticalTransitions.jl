...
@testset "Self-consistent reactive rate" begin
    ...
    @test isapprox(k_outA, k_intoB; rtol = 1.0e-6)
end
...
@testset "3D dispatch" begin
    ...
    @test maximum(abs, vec(sum(res.generator.Q; dims = 2))) < 1.0e-10
...
@testset "Probability accessors" begin
    ...
    @test probability_last_A(res) ≈ 0.5 atol = 1.0e-10
end
...
@testset "Physical reverse drift" begin
    ...
    @test maximum(abs.(res.qminus .- res_chain.qminus)) < 1.0e-6
end
...
@testset "ReactiveTransition with Krylov solver" begin
    ...
    @test reactive_rate(res_kr) ≈ reactive_rate(res_lu) rtol = 1.0e-8
end
...
@testset "Analytical: P(last A) + P(last B) = 1" begin
    ...
    @test p_last_A + p_last_B ≈ 1.0 atol = 1.0e-12
end
...
@testset "Analytical: 1D reactive current is constant and equals k_AB" begin
    ...
    @test maximum(interior) - minimum(interior) < 1.0e-10
    @test sum(interior) / length(interior) * grid.h[1] ≈ reactive_rate(res) rtol = 1.0e-10
end
