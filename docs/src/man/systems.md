...
\end{aligned}
```  
where ```P_1, P_0, Z_1, Z_0``` and ```t_0``` are constants to be determined in the non-dimensionalisation process. We take ```P_0 = h, Z_0 = a/m, t_0 = 1/a``` and subsequently introduce the following dimensionless parameters ```\alpha := r/a, \beta := h/k, \gamma := a/(mh)```. The values of the (dimensionless) constants ```P_1``` and ```Z_1``` are tuned according to the user's preference; they can be chosen in  a way such that the fixed points of the system are all contained within the ```[0,1]x[0,1]``` subspace, for instance.  
