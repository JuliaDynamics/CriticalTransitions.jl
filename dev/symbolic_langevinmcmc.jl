...
function langevinmcmc_spde(u, p, t)
    ...
    for ii in 2:(N - 1)
        dzxii = [central(u[((jj - 1) * N + 1):(jj * N)], ii, Δz) for jj in 1:dim] # the derivative ∂x/∂z at physical time z = ii*Δz 
        dzzxii = [central2(u[((jj - 1) * N + 1):(jj * N)], ii, Δz) for jj in 1:dim] # the derivative ∂^2x/∂z^2 at physical time z = ii*Δz
        xii = [u[(jj - 1) * N + ii] for jj in 1:dim] # the state variable state_vars at z = ii*Δz
        ...
        du[[(jj - 1) * N + ii for jj in 1:dim]] =
            inv(Σ) * dzzxii - (inv(Σ) * Jxii - Jxii' * inv(Σ)) * dzxii - (1 / 2) * fxii -
            (σ^2 / 2) * gxii # the deterministic portion of the SPDE function evaluated at xii
    end
...
function symbolise_spde(sys::StochSystem)
    ...
    state_vars = @eval @variables $([Symbol("x$i") for i in 1:length(sys.u)]...)
...
function jacobian(sys::StochSystem)
    ...
    # next, the jacobian of the drift in symbolic form
    return jacobian = Symbolics.jacobian(drift, state_vars)
end
...
function langevinmcmc(
    ...
    p = [
        length(sys.u), sys.σ, sys.Σ, Δz, state_vars, jacobian, grad_dot_term, grad_div_term
    ] # langevinmcmcSPDE parameters 
    ...
    for it in 1:Nstep
        ...
        for jj in 1:length(sys.u)
            update_mod[jj, :] = update[((jj - 1) * N + 1):(jj * N)]
        end

        paths[it + 1, :, :] =
            paths[it, :, :] .+
            (Δt * update_mod .+ _noise * sqrt(2 * Δt / Δz) * randn(length(sys.u), N)) # the Euler-Maruyama step 
...
function langevinmcmc_not_every_step(
    sys::StochSystem,
    init;
    T=15,         # physical end time
    tmax=250.0,   # virtual end time
    ...
    for it in 1:Nstep
        ...
        pathstemp[:, :] =
            pathstemp[:, :] .+
            (Δt * update_mod .+ sys.σ * √(2 * Δt / Δz) * randn(length(sys.u), N)) # the Euler-Maruyama step 
...
function stochastic_bridge(sys::StochSystem, Tphys::Float64, Δz::Float64)
    ...
    p = vcat(
        [length(sys.u), sys.σ, sys.Σ, Δz],
        [symbolise_spde(sys)[i] for i in 1:length(symbolise_spde(sys))],
    ) # defining the p-field of StochSystem
    dim = length(sys.u) * N # the dim-field of StochSystem
    σ = √(2 / Δz) * sys.σ # defining the σ-field of StochSystem

    z = ones(dim)
    z[reduce(vcat, [[(ii - 1) * N + 1, ii * N] for ii in 1:length(sys.u)])] .= zeros(
        2 * length(sys.u)
    ) # this is a modified version of idfunc with zeros at the beginning and end of each length(sys.u) portion
    ...
    return StochSystem(f, p, dim, σ, g, pg, Σ, process)
end
