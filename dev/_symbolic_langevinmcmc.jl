...
function langevinmcmc_spde(u, p, t)
    ...
    for ii in 2:(N - 1)
        dzxii = [central(u[((jj - 1) * N + 1):(jj * N)], ii, Δz) for jj in 1:dim] # the derivative ∂x/∂z at physical time z = (ii-1)*Δz 
        dzzxii = [central2(u[((jj - 1) * N + 1):(jj * N)], ii, Δz) for jj in 1:dim] # the derivative ∂^2x/∂z^2 at physical time z = ii*Δz
        xii = [u[(jj - 1) * N + ii] for jj in 1:dim] # the state variable state_vars at z = (ii-1)*Δz
        ...
        du[[(jj - 1) * N + ii for jj in 1:dim]] =
            inv(Q) * dzzxii - (inv(Q) * Jxii - Jxii' * inv(Q)) * dzxii - (1 / 2) * fxii -
            (σ^2 / 2) * gxii
    end
