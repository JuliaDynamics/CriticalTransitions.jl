...
function meanresidencetime(
    ...
    axs1 = Axis(
        fig[1, 1];
        title="Iterative mean transition time",
        xlabel="Sample size",
        ylabel="Mean transition time",
    )
    ...
    while ~converged
        ...
        lastMmeans .=
            mean(finaltimes) .- meanresidencetimes[setdiff(1:(M + 1), [mrt_position])]
...
function meantransitiontime(
    ...
    times = transitions(sys, x_i, x_f, sample_size; rad_i, rad_f, showprogress=false)[2]
    transitiontimes = [times[i][end] - times[i][1] for i in 1:length(times)]

    return mean(transitiontimes)
end
...
function meanresidencetime(
    ...
    residencetimes = [times[i][end] for i in 1:length(times)]
...
fps = fixedpoints(fhn, bmin, bmax);
sfps = fps[1][fps[3]];
x_i = [sfps[1, 1], sfps[1, 2]];
x_f = [sfps[2, 1], sfps[2, 2]];
